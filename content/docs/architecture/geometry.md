...
Each canvas has its origin at the top left (0, 0) corner. Every element of the UI may be scaled depending on the output device, so the API does not describe pixels or exact measurements.
...
Every position referenced by a CanvasObject is relative to its parent.
...
To manage this complexity the toolkit manages scaling internally so
your apps will always look the right size. 
If a user sets the scale to be smaller, then their apps will always have
smaller than normal fonts, buttons etc. When the user specifies a larger scale,
your app will scale up to suit.

When compared to [Material Design](https://material.io) we can see that
their baseline DPI is [160](https://material.io/design/layout/pixel-density.html#pixel-density-on-android), so although the calculation is similar, the 
...
One time that pixel sizes will matter is if you start loading bitmap images. Normally these scale appropriately, but if you specify
